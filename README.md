...
- CI also runs `make coverage-check` to generate `coverage.xml` from SwiftPM/LCOV model coverage and fail below 85%; SonarCloud uses the same model-layer coverage scope when `SONAR_TOKEN` is available.
